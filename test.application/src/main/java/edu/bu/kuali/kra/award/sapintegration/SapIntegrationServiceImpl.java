 ...
 */
package edu.bu.kuali.kra.award.sapintegration;
...
import edu.bu.kuali.kra.bo.AwardTransmissionChild;
import edu.bu.sap.kcrm.SIKCRMPROCESSOUTBOUND;
import edu.bu.sap.kcrm.SIKCRMPROCESSOUTBOUNDService;

/**
 * <p>A service implementation which provides integration with Boston University's SAP implementation.
...
public class SapIntegrationServiceImpl implements SapIntegrationService, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        if (awardAmountInfoService == null) {
            throw new IllegalStateException("The awardAmountInfoService was not injected.");
        }
        if (unitService == null) {
            throw new IllegalStateException("The unitService was not injected.");
        }
        if (awardHierarchyService == null) {
            throw new IllegalStateException("The awardHierarchyService was not injected.");
        }
        if (businessObjectService == null) {
            throw new IllegalStateException("The businessObjectService was not injected.");
        }
        if (budgetRateAndBaseService == null) {
            throw new IllegalStateException("The budgetRateAndBaseService was not injected.");
        }
        if (rolodexService == null) {
            throw new IllegalStateException("The rolodexService was not injected.");
        }

    }

    public String getTransmitXml(SapTransmission transmission) {
        ZGMKCRMINTERFACE sapInterface = constructSapInterface(transmission, new ArrayList<Long>());
        StringWriter stringWriter = new StringWriter();
        try {
            JAXBContext context = JAXBContext.newInstance(ZGMKCRMINTERFACE.class);
...
//        LOG.info(stringWriter.toString()); //TODO mkousheh delete once in production
    ...
    public SapTransmissionResponse transmit(SapTransmission transmission) {
        ...
        if (!results.calculateSuccess()) {
            return SapTransmissionResponse.validationFailure("Failed to validate transmission successfully.");
        }

        return executeSapService(transmission);
    }

    protected ZGMKCRMINTERFACE constructSapInterface(SapTransmission transmission, List<Long> interfacedSponsoredProgramIds) {
        ...
        AwardSponsorContact billingPartner = determineBillingPartner(transmission.getAward());
        if (billingPartner != null) {
            ZGMSPONSORSTRUCTURE billingPartnerSponsor = constructSponsorFromBillingPartner(transmission.getAward(), billingPartner);
            sponsors.getItem().add(billingPartnerSponsor);
        }

        ZGMBILLINGPLANSTRUCTURE billingPlan = constructBillingPlan(transmission.getAward());
        if (billingPlan != null) {
            billingPlans.getItem().add(billingPlan);
        }

        SPONSOREDPROGRAMS sponsoredPrograms = objectFactory.createZGMKCRMINTERFACESPONSOREDPROGRAMS();
        for (Award childAward : transmission.getChildAwards()) {
            if (determineCostShareMemoMatch(childAward) == null) {
                if (!"group".equalsIgnoreCase(((AwardExtension) childAward.getExtension()).getChildType())) {
                    ZGMSPPROGRAMSTRUCTURE sponsoredProgram = constructSponsoredProgram(childAward, false);
                ...
                if (childAward.getPrimeSponsor() != null) {
                    ZGMSPONSORSTRUCTURE childAwardPrimeSponsor = constructSponsor(childAward, childAward.getPrimeSponsor());
                    sponsors.getItem().add(childAwardPrimeSponsor);
                }

                for (AwardApprovedSubaward awardApprovedSubaward : childAward.getAwardApprovedSubawards()) {
                    ZGMSPONSORSTRUCTURE subAwardOrgSponsor = constructSponsorFromOrganization(childAward, awardApprovedSubaward.getOrganization());
        ...
        ZGMSPPROGRAMSTRUCTURE costSharingSponsoredProgram = processCostSharing(transmission.getAward());
        if (costSharingSponsoredProgram != null) {
            sponsoredPrograms.getItem().add(costSharingSponsoredProgram);
            interfacedSponsoredProgramIds.add(transmission.getAward().getAwardId());
        }

        SPONSOREDPROGRAMSGRP sponsoredProgramsGrp = constructSponsoredProgramGroups(transmission.getAward());
    ...
    protected SapTransmissionResponse executeSapService(SapTransmission transmission) {
        ...
        SIKCRMPROCESSOUTBOUND sapService = newWebServicePort(new PrintWriter(sendWriter), new PrintWriter(receiveWriter));
        ...
        try {
            ...
            if (nestedCause instanceof SocketTimeoutException) {
                ...
                return SapTransmissionResponse.transmissionFailure(nestedCause.getMessage(), null, sendWriter.toString(), receiveWriter.toString());
            }
        ...
        if (failureMessage != null) {
            return SapTransmissionResponse.transmissionFailure(failureMessage, warningMessages, sendWriter.toString(), receiveWriter.toString());
        }
        ...
        try {
            sponsoredProgramIds = extractSponsoredProgramIds(interfacedSponsoredProgramIds, transmission, sponsoredProgramsMessages);
        ...
        // BU Customization ID: N/A mukadder 20130429 - ENHC0010154 - Issue 55 - KC_SAP Interface to display warning message
        return SapTransmissionResponse.success(sponsoredProgramIds, walkerIds, warningMessages, sendWriter.toString(), receiveWriter.toString());
    }

    /**
     * Extract Walker numbers from the return message
     */
    private Map<Long, String> extractWalkerIds(List<Long> interfacedSponsoredProgramIds, SapTransmission transmission, SPXWALKT walkerMessages) {
        Map<Long, String> walkerIds = new HashMap<Long, String>();
        if (walkerMessages != null && walkerMessages.getItem() != null) {
            if (walkerMessages.getItem().size() != interfacedSponsoredProgramIds.size()) {
                throw new IllegalStateException("The number of sponsored program messages returned should be equal to the number of sponsored programs interfaced.  " +
                        "Instead " + interfacedSponsoredProgramIds.size() + " sponsored programs were interfaced and " +
                        walkerMessages.getItem().size() + " sponsored program messages were returned.");
            }
            for (int index = 0; index < walkerMessages.getItem().size(); index++) {
                ZFIKCRMSPXWALK walkerMessage = walkerMessages.getItem().get(index);
     ...
     */
    protected SIKCRMPROCESSOUTBOUND newWebServicePort(PrintWriter sendWriter, PrintWriter receiveWriter) {
        String sapWebServiceUrlValue = ConfigContext.getCurrentContextConfig().getProperty(SAP_SERVICE_WSDL_URL_PARAM);
        if (StringUtils.isBlank(sapWebServiceUrlValue)) {
            throw new IllegalStateException("The " + SAP_SERVICE_WSDL_URL_PARAM + " configuration parameter needs to be set in order to invoke the service.");
        }
        try {
            URL sapWebServiceUrl = new URL(sapWebServiceUrlValue);
            // TODO could probably cache the created ZFIGMKCRMINTERFACE_Service
            SIKCRMPROCESSOUTBOUND sapService = new SIKCRMPROCESSOUTBOUNDService(sapWebServiceUrl).getSIKCRMPROCESSOUTBOUNDPort();
            configureServiceEndpoint(sapService, sendWriter, receiveWriter);
            return sapService;
        } catch (MalformedURLException e) {
            throw new RuntimeException("Failed to initialize SAP web service url because it was invalid, value was: " + sapWebServiceUrlValue, e);
        }
    }

    /**
     * Configures the service endpoint by configuring basic http authentication on the service as well as setting a custom value for the
     * service endpoint if one has been configured.
     */
    protected void configureServiceEndpoint(SIKCRMPROCESSOUTBOUND serviceEndpoint, PrintWriter sendWriter, PrintWriter receiveWriter) {
        ...
        String connectionTimeout = ConfigContext.getCurrentContextConfig().getProperty(SAP_SERVICE_CONNECTION_TIMEOUT_PARAM);
        String receiveTimeout = ConfigContext.getCurrentContextConfig().getProperty(SAP_SERVICE_RECEIVE_TIMEOUT_PARAM);
        ...
        if (!(serviceEndpoint instanceof BindingProvider)) {
            throw new IllegalArgumentException("The given service endpoint should be an instance of BindingProvider but was not.");
        }
        ...
        if (StringUtils.isBlank(username)) {
            throw new IllegalStateException("No username was configured for the SAP service, please ensure that the following configuration parameter is set: " + SAP_SERVICE_USERNAME_PARAM);
        }
        if (StringUtils.isBlank(password)) {
            throw new IllegalStateException("No passwrod was configured for the SAP service, please ensure that the following configuration parameter is set: " + SAP_SERVICE_PASSWORD_PARAM);
        }
    ...
    /**
     * In the case of warning, returns a non-null string that contains the warning message.
     * BU Customization ID: N/A mukadder 20130429 - ENHC0010154 - Issue 55 - KC_SAP Interface to display warning message
     */
    private List<String> processWarningMessages(ZGMKCRMINTERFACEResponse response) {
    ...
    /**
     * In the case of failure, returns a non-null string that contains the failure message.
     */
            ...
            if (grantMessages != null && grantMessages.getItem() != null) {
                for (ZGRANTMESSAGES grantMessage : grantMessages.getItem()) {
                    if (ERROR_MESSAGE_TYPE.equals(grantMessage.getTYPE())) {
                        isSuccess = false;
                        failureMessage.append("    [Grant Failure Message: " + grantMessage.getGRANTNBR() + "] - " + grantMessage.getMESSAGE() + "\n");
                    } else {
                        hasSuccessMessage = true;
                        successMessage.append("    [Grant Success Message: " + grantMessage.getGRANTNBR() + "] - " + grantMessage.getMESSAGE() + "\n");
                    }
                }
            }

            if (sponsorMessages != null && sponsorMessages.getItem() != null) {
                for (ZSPONSORMESSAGES sponsorMessage : sponsorMessages.getItem()) {
                    if (ERROR_MESSAGE_TYPE.equals(sponsorMessage.getTYPE())) {
                        isSuccess = false;
                        failureMessage.append("    [Sponsor Failure Message: " + sponsorMessage.getSPONSOR() + "] - " + sponsorMessage.getMESSAGE() + "\n");
                    } else {
                        hasSuccessMessage = true;
                        successMessage.append("    [Sponsor Success Message: " + sponsorMessage.getSPONSOR() + "] - " + sponsorMessage.getMESSAGE() + "\n");
                    }
                }
            }

            if (sponsoredProgramsMessages != null && sponsoredProgramsMessages.getItem() != null) {
                for (ZSPPROGRAMMESSAGES sponsoredProgramsMessage : sponsoredProgramsMessages.getItem()) {
                    if (ERROR_MESSAGE_TYPE.equals(sponsoredProgramsMessage.getTYPE())) {
                        isSuccess = false;
                        failureMessage.append("    [Sponsored Program Failure Message: " + sponsoredProgramsMessage.getSPONSOREDPROG() + "] - " + sponsoredProgramsMessage.getMESSAGE() + "\n");
                    } else {
                        hasSuccessMessage = true;
                        successMessage.append("    [Sponsored Program Success Message: " + sponsoredProgramsMessage.getSPONSOREDPROG() + "] - " + sponsoredProgramsMessage.getMESSAGE() + "\n");
                    }
                }
            }
    ...
    protected ZGMGRANTSTRUCTURE constructGrant(Award award) {
        ...
        AwardAmountInfo awardAmountInfo = getAwardAmountInfoService().fetchLastAwardAmountInfoForAwardVersionAndFinalizedTandMDocumentNumber(award);

        // Position 8
        grantHeaderAdd.setGRANTTOTAL(awardAmountInfo.getAmountObligatedToDate().bigDecimalValue());

        // Position 9
        grantHeader.setVALIDFROM(dateToString( org.kuali.coeus.sys.framework.util.DateUtils.convertToSqlDate(awardAmountInfo.getCurrentFundEffectiveDate())));
        ...
        if (awardAmountInfo.getCurrentFundEffectiveDate() != null) {
        	java.util.Date tmpDate = org.apache.commons.lang3.time.DateUtils
					...
					.addMonths(awardAmountInfo.getCurrentFundEffectiveDate(),
							-3);
            grantHeader.setVALIDFROMBUDGET(dateToString( org.kuali.coeus.sys.framework.util.DateUtils.convertToSqlDate(tmpDate)));
        }

        // Position 10
        grantHeader.setVALIDTO(dateToString( org.kuali.coeus.sys.framework.util.DateUtils.convertToSqlDate(awardAmountInfo.getObligationExpirationDate())));
        ...
        if (awardAmountInfo.getObligationExpirationDate() != null) {
            ...
            grantHeader.setVALIDTOBUDGET(dateToString( org.kuali.coeus.sys.framework.util.DateUtils.convertToSqlDate(tmpDate2)));
        }
        ...
        // Position 14
        grantHeaderAdd.setLETTEROFCREDIT(convertMethodOfPaymentToLetterOfCredit(award.getMethodOfPaymentCode(), award));
        ...
        for (AwardReportTerm awardReportTermItem : award.getAwardReportTermItems()) {
            ...
            if (!awardReportTermItem.getReportCode().equals("27") && !awardReportTermItem.getReportCode().equals("24")) {
                if (PAYMENT_INVOICES_REPORT_CLASS_CODE.equals(awardReportTermItem.getReportClassCode())) {
                    grantData.setZZINVOICEFREQ(convertFrequencyCodeToInvoiceFrequency(awardReportTermItem.getFrequencyCode()));
        ...
        // Position 36
        if (getMostRecentAwardReportTerm(award) != null && getMostRecentAwardReportTerm(award).getAwardReportTermRecipients().size() > 0) {
            grantData.setZZBILLPARTNER(getMostRecentAwardReportTerm(award).getAwardReportTermRecipients().get(0).getRolodexId().toString());
        }
        ...
        for (AwardPerson awardPerson : award.getProjectPersons()) {
            ...
            if (awardPerson.isMultiplePi()) {
                ...
                grantPerson.setRESPONSIBILITY(convertProjectRoleToResponsibility(awardPerson.getContactRoleCode(), award));
     ...
     * <p/>
     * For example, if award number is 123456-00001 then grant number will be 123456.
     *
     * @param awardNumber the award number
     * @return the derived grant number
     * @throws IllegalArgumentException if the award number is not in a valid format
     */
    private String deriveGrantNumber(String awardNumber) {
        if (StringUtils.isBlank(awardNumber) || awardNumber.length() < 6) {
            throw new IllegalArgumentException("Grant number could not be derived from the given award number, it was either blank or less than 6 digits.  Award number was: " + awardNumber);
        }
        return awardNumber.substring(0, 6);
    }

    /**
     * Converts the given frequency code from KC to an SAP invoice frequency according to the following mapping:
     * <p/>
     * When Frequency_Code = 1, Invoice_Frequency = "M�
     * When Frequency_Code = 2, Invoice_Frequency = "Q"
     * All other values for Frequency_Code, Invoice_Frequency = "O"
     *
     * @param frequencyCode the frequencyCode to convert
     * @return the resulting invoice frequency
     * @throws IllegalArgumentException if the given frequencyCode is blank or null
     */
    private String convertFrequencyCodeToInvoiceFrequency(String frequencyCode) {
        if (StringUtils.isBlank(frequencyCode)) {
            throw new IllegalArgumentException("The given frequencyCode was null or blank, could not convert to an invoice frequency.");
        }
    ...
    /**
     * Converts the given report code from KC to an SAP invoice form according to the following mapping:
     * <p/>
     * When Report_Code = 1, Invoice_Form = "ZFI_GM_SF425"
     * When Report_Code = 3, Invoice_Form = "ZFI_GM_SF270"
     * When Report_Code = 25, Invoice_Form = "ZFI_GM_SF1034-1035"
     * When Report_Code = 26, Invoice_Form = "ZFI_GM_BU-Standard"
     * When Report_Code = 28, Invoice_Form = "Manual"
     */
    private String convertReportCodeToInvoiceForm(String reportCode) {
        String invoiceForm = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.REPORT_CODE_TO_INVOICE_FORM_MAPPING, reportCode);
        if (StringUtils.isEmpty(invoiceForm)) {
            throw new IllegalArgumentException("Could not resolve an invoice form from the given report code, value was: " + reportCode);
        }
        return invoiceForm;
    }
     ...
     * <p/>
     * When a row exists on AWARD_REPORT_TERMS where AWARD_REPORT_TERMS.REPORT_CLASS_CODE = 8 and
     * AWARD_REPORT_TERMS.REPORT_CODE = 27 � Send an "X" value for Advance_Payment.  Otherwise don"t send a value.
     *
     * @param award the award to determine the advance payment for
     * @return the value of the advance payment ("X") or null if no advance payment data should be sent
     */
    private String determineAdvancePayment(Award award) {
        ...
        for (AwardReportTerm awardReportTerm : awardReportTerms) {
            if (PAYMENT_INVOICES_REPORT_CLASS_CODE.equals(awardReportTerm.getReportClassCode()) && "27".equals(awardReportTerm.getReportCode())) {
                return "X";
            }
        }
        return null;
    ...
    /**
     * Determines the Property_Owner_Title for the given Award using the following mapping:
     * <p/>
     ...
     *
     * @param award the award to determine the property owner title for
     ...
     * <p/>
     * If an Award has a row on the AWARD_COST_SHARE where Cost_Share_Type_Code = 3, then Cost_Share_Memo_Match = "X".
     * Otherwise don't send a value.
     *
     * @param award the award to determine the cost share memo match for
     * @return the value of the cost share memo match ("X") or null if no cost share memo match data should be sent
     */
     ...
     *
     * @param awardTypeCode the awardTypeCode to convert
     * @return the avc tolerance value for the given indicator
     */
    private String convertAwardTypeCodeToAwardType(int awardTypeCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.AWARD_TYPE_MAPPING, Integer.toString(awardTypeCode));
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert award type, given award type value was not understood: " + awardTypeCode + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
     ...
     *
     * @param avcIndicator the avcIndicator to convert
     * @return the avc tolerance value for the given indicator
     */
    private String convertAvcIndicatorToAvcTolerance(String avcIndicator) {
        if (StringUtils.isBlank(avcIndicator)) {
            throw new IllegalArgumentException("The avc indicator was blank or null.");
        }
        ...
        throw new IllegalArgumentException("Could not resolve avc tolerance from the given avc indicator, value was: " + avcIndicator);
    }

    /**
     * Takes the given Interest Earned Code and converts it to an Interest Earned Code tolerance.
     *
    ...
    private String convertInterestEarnedCode(String interestEarnedCode) {
        ...
        throw new IllegalArgumentException("Could not convert the given interest earned code, value was: " + interestEarnedCode);
    }

    /**
     * Determines the Billing Partner ID value if found, otherwise use the Rolodex ID
     *
     * @param award the award to determine the billing partner match for
    ...
    /**
     * @param award award the award to determine the billing partner match for
    ...
    /**
     * Implements section 1.7.7 of the functional specification to create sponsored term data.
     * It does this for child awards as well as their parent.
     * <p/>
     * <p>Note that if an award has multiple sponsored terms, they all should be sent (not only
     * the most recent).
     */
    protected ZBAPI0035SPONSOREDOBJECTST constructSponsoredObjects(SapTransmission transmission) {
        ZBAPI0035SPONSOREDOBJECTST sapSponsoredObjects = objectFactory.createZBAPI0035SPONSOREDOBJECTST();
        Set<SponsoredObject> parentSponsoredObjects = constructSponsoredObjects(null, transmission.getAward());
        if (parentSponsoredObjects != null && isCostSharing(transmission.getAward())) {
        ...
        for (Award award : transmission.getChildAwards()) {
            if (!"group".equalsIgnoreCase(((AwardExtension) award.getExtension()).getChildType())) {
                Set<SponsoredObject> sponsoredObjects = constructSponsoredObjects(parentSponsoredObjects, award);
    ...
    protected Set<SponsoredObject> constructSponsoredObjects(Set<SponsoredObject> parentSponsoredObjects, Award award) {
        ...
        if (awardSponsorTerms != null) {
            for (AwardSponsorTerm awardSponsorTerm : awardSponsorTerms) {
                String sponsoredClass = convertSponsorTermToSponsorClass(awardSponsorTerm.getSponsorTermId(), award);
                if (!StringUtils.isBlank(sponsoredProgram) && !StringUtils.isBlank(sponsoredClass)) {
                    sponsoredObjects.add(new SponsoredObject(sponsoredProgram, sponsoredClass));
                }
            }
        }
        // copy in the entries from the parent, using the child sponsored program
        if (parentSponsoredObjects != null) {
            for (SponsoredObject parentSponsoredObject : parentSponsoredObjects) {
                sponsoredObjects.add(new SponsoredObject(sponsoredProgram, parentSponsoredObject.sponsoredClass));
            }
        }
        return sponsoredObjects;
    }

    protected void loadSapSponsoredObjects(ZBAPI0035SPONSOREDOBJECTST sapSponsoredObjects, Set<SponsoredObject> sponsoredObjects) {
        for (SponsoredObject sponsoredObject : sponsoredObjects) {
            ZBAPI0035SPONSOREDOBJECTS sapSponsoredObject = objectFactory.createZBAPI0035SPONSOREDOBJECTS();
            sapSponsoredObject.setSPONSOREDPROG(sponsoredObject.sponsoredProgram);
            sapSponsoredObject.setSPONSOREDCLASS(sponsoredObject.sponsoredClass);
            sapSponsoredObjects.getItem().add(sapSponsoredObject);
        }
    }

    /**
     * This class is just a wrapper around sponsored program and sponsored class.  Implements hashcode and equals
     * method which allows us to use it inside a Set and prevent duplicate entries from happenign.
     */
    private static class SponsoredObject {
        ...
        SponsoredObject(String sponsoredProgram, String sponsoredClass) {
            if (StringUtils.isBlank(sponsoredProgram) || StringUtils.isBlank(sponsoredClass)) {
                throw new IllegalArgumentException("both sponsored program and sponsored class must be non-null!");
            }
        ...
        public boolean equals(Object object) {
            if (object instanceof SponsoredObject) {
                ...
                return sponsoredObject.sponsoredProgram.equals(sponsoredProgram) && sponsoredObject.sponsoredClass.equals(sponsoredClass);
            }
            return false;
        }
    ...
    protected ZGMSPPROGRAMSTRUCTURE constructSponsoredProgram(Award award, boolean costSharing) {
        ...
        if (!awardBudgetVersionToBePosted) {
            ...
            AwardTransmissionChild lastTransmittedChild = getLatestChildAwardTransmitted(hierarchy.getRoot().getAward(), award.getAwardNumber(), true);
            ...
            BigDecimal fAndARate = calculateApplicableFandARate(award);
            if (fAndARate != null && (((AwardExtension) award.getExtension()).getSteppedUpRate() == null || "NA".equalsIgnoreCase(((AwardExtension) award.getExtension()).getSteppedUpRate()))) {
                ...
                overheadKey = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.FEDERAL_RATE_DATE_OVERHEAD_KEY_FIELD_MAPPINGS, ((AwardExtension) award.getExtension()).getSteppedUpRate());
        ...
        GlobalVariables.getUserSession().addObject("overheadKey-".concat(award.getAwardNumber()), (Object) overheadKey);
        ...
        if (!costSharing) {
            ...
            if (budget == null) {
                throw new IllegalArgumentException("Award with number " + award.getAwardNumber() + " does not have a budget.");
            }
        }
        ...
        if (costSharing) {
            ...
            if (tmpAwardCostShare != null && ((AwardExtension) award.getExtension()).getLastTransmissionDate() != null && tmpAwardCostShare.getUpdateTimestamp() != null && ((AwardExtension) award.getExtension()).getLastTransmissionDate().before(tmpAwardCostShare.getUpdateTimestamp())) {
...
//        }
        ...
        if (hierarchy.getRoot().getAward().getAwardTransactionTypeCode() != null) {
            if (hierarchy.getRoot().getAward().getAwardTransactionTypeCode().equals(10) || hierarchy.getRoot().getAward().getAwardTransactionTypeCode().equals(6)) {
                sponsoredProgramStructure.setBUDGETTDC(ScaleTwoDecimal.ZERO.bigDecimalValue());
                sponsoredProgramStructure.setBUDGETFA(ScaleTwoDecimal.ZERO.bigDecimalValue());
            }
        }
        ...
        sponsoredProgramStructure.setFRINGECODE(FringeCodeMapping.mapToSapFringeCode(fringeNotAllowed, award.getAccountTypeCode()));
        ...
        if (!costSharing && !hierarchy.hasChildren()) {
            ...
            GlobalVariables.getUserSession().addObject("offCampus-".concat(award.getAwardNumber()), (Object) offCampus);
            ...
            GlobalVariables.getUserSession().addObject("baseCode-".concat(award.getAwardNumber()), (Object) baseCode);
            ...
            for (AwardPerson person : persons) {
                ...
                if (person.isMultiplePi()) {
                    ...
                    sponsoredProgramPerson.setRESPONSIBILITY(convertProjectRoleToResponsibility(person.getContactRoleCode(), award));
        ...
        if (costSharing && costSharCommitmentMinusMemoMatch != null && costSharCommitmentMinusMemoMatch.isPositive()) {
    ...
    private ScaleTwoDecimal getTotalCostShareCommitmentAmountMinusMemoMatch(Award award) {
    ...
    /**
     * Calculates the applicable F and A rate for the given Award using the current system time as the effective date.
     *
     * @param award the award for whic to calculate the rate
     * @return the value of the rate as a BigDecimal
     * @throws IllegalStateException if the rate could not be found for the current date/time
     */
    private BigDecimal calculateApplicableFandARate(Award award) {
    	ScaleTwoDecimal fAndARate = getBudgetRateAndBaseService().calculateApplicableFandARate(award);
        if (fAndARate == null) {
            return null;
        }
    ...
    /**
     * Converts the given child type to a format that SAP can understand according to the following mapping:
     * <p/>
     * If "Standard�; Child_Type = ST1
     * If "Fabrication", Child_Type =  P01
     * If "Participant Support Costs", Child_Type = PS1
     * If "Subaward", Child_Type = SA1
     *
     * @param childType the child type to convert
     * @return the converted child type value
     */
    private String convertChildType(String childType, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.CHILD_TYPE_MAPPING, childType);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert child type, given child type value was not understood: " + childType + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
     ...
     * <p/>
     * A sponsored program for cost sharing will only get created if the award which is passed in supports cost sharing.
     *
     * @return the sponsored program information for the cost sharing award, or null if the award does not support cost sharing
     */
    protected ZGMSPPROGRAMSTRUCTURE processCostSharing(Award award) {
        ScaleTwoDecimal costSharCommitmentMinusMemoMatch = getTotalCostShareCommitmentAmountMinusMemoMatch(award);
        if (isCostSharing(award) && costSharCommitmentMinusMemoMatch.isPositive()) {
    ...
    /**
     * Cost Sharing Exists when data for an Award exists on Award_Cost_Share and the Commitment Amount > 0
     */
    protected boolean isCostSharing(Award award) {
        AwardCostShare awardCostShare = getMostRecentAwardCostShare(award);
        if (awardCostShare != null) {
            if (awardCostShare.getCommitmentAmount() != null && awardCostShare.getCommitmentAmount().isPositive()) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Implements section 1.7.9 of functional specification which relates to Billing Plans.
     */
    protected ZGMBILLINGPLANSTRUCTURE constructBillingPlan(Award award) {
        ...
        ZGMBILLINGPLANSTRUCTURE billingPlan = objectFactory.createZGMBILLINGPLANSTRUCTURE();
    ...
    /**
     * Implements section 1.8 of functional spec related to Sponsor data mapping.
     */
    ...
    protected ZGMSPONSORSTRUCTURE constructSponsor(Award award, Sponsor sponsor) {
        ...
        ZGMSPONSORSTRUCTURE sponsorStructure = objectFactory.createZGMSPONSORSTRUCTURE();

        // Position 1
        sponsorStructure.setSPONSOR(sponsor.getSponsorCode());

        // Position 2
        sponsorStructure.setSPONSORTYPE(convertSponsorCodeToSponsorType(sponsor.getSponsorTypeCode(), award));

        // Position 3
        sponsorStructure.setSPONSORNAME(sponsor.getSponsorName());

        // Position 4
        sponsorStructure.setCONTACTPERSON(rolodex.getFirstName() + " " + rolodex.getLastName());
        ...
        // BU Customization ID: N/A mkousheh 20110712 - Special case where a Prime Sponsor present then use
     ...
     */
    protected ZGMSPONSORSTRUCTURE constructSponsorFromOrganization(Award award, Organization organization) {

        Sponsor sponsor = businessObjectService.findBySinglePrimaryKey(Sponsor.class, organization.getOrganizationId());
        if (sponsor == null) {
            throw new IllegalStateException("Failed to locate a Sponsor record for the given organization with id: " + organization.getOrganizationId());
        }
    ...
    /**
     * Construct a sponsor structure from an AwardSponsorContact representing the billing partner
     */
    protected ZGMSPONSORSTRUCTURE constructSponsorFromBillingPartner(Award award, AwardSponsorContact billingPartner) {

    	RolodexContract rolodex = billingPartner.getRolodex();

        if (rolodex == null) {
            if (billingPartner.getRolodexId() != null) {
                rolodex = getRolodexService().getRolodex(billingPartner.getRolodexId());
            }
            if (rolodex == null) {
                throw new IllegalStateException("Billing partner does not have a rolodex entry.  Award contact id of billing partner is: " + billingPartner.getAwardContactId() + ", rolodex id is: " + billingPartner.getRolodexId());
            }
        }
        ...
        // Position 2
        sponsorStructure.setSPONSORTYPE(DEFAULT_SPONSOR_TYPE_FOR_BILLING_PARTNER);

        // Position 3
        sponsorStructure.setSPONSORNAME(billingPartner.getContactOrganizationName());
    ...
    private AwardPersonUnitCreditSplit getMostRecentCreditSplit(AwardPersonUnit awardPersonUnit) {
        List<AwardPersonUnitCreditSplit> creditSplits = awardPersonUnit.getCreditSplits();
        if (creditSplits.isEmpty()) {
            // credit split panel not being used any more
            //            throw new IllegalStateException("Person unit has no credit splits.");
            return null;
        }
    ...
    private SPONSOREDPROGRAMSGRP constructSponsoredProgramGroups(Award award) {
        ...
        SPONSOREDPROGRAMSGRP programGroups = objectFactory.createZGMKCRMINTERFACESPONSOREDPROGRAMSGRP();
        AwardHierarchy hierarchy = getAwardHierarchyService().loadAwardHierarchy(award.getAwardNumber());
        for (AwardHierarchy childHierarchy : hierarchy.getChildren()) {
            if ("Group".equalsIgnoreCase(((AwardExtension) award.getExtension()).getChildType())) {
//                ZGMSPPROGRAMGRPSTRUCTURE programGroup = objectFactory.createZGMSPPROGRAMGRPSTRUCTURE();
...
//                programGroups.getItem().add(programGroup);
    ...
    private void generateProgramGroupChildren(SPONSOREDPROGRAMSGRP programGroups, AwardHierarchy awardHierarchy) {
        for (AwardHierarchy childHierarchy : awardHierarchy.getChildren()) {
            ZGMSPPROGRAMGRPSTRUCTURE childProgramGroup = objectFactory.createZGMSPPROGRAMGRPSTRUCTURE();
            childProgramGroup.setPARENTAWARD(childHierarchy.getParentAwardNumber());
            childProgramGroup.setCHILDAWARD(childHierarchy.getAwardNumber());
            childProgramGroup.setTITLE(childHierarchy.getAward().getTitle());
            childProgramGroup.setSPONSOREDPROG(childHierarchy.getAward().getAccountNumber());
            programGroups.getItem().add(childProgramGroup);
            if ("Group".equalsIgnoreCase(((AwardExtension) (childHierarchy.getAward().getExtension())).getChildType()) && childHierarchy.hasChildren()) {
                generateProgramGroupChildren(programGroups, childHierarchy);
            }
        }
    }
    ...
    /**
     * Extracts sponsored program ids from the given sponsored programs message responses.
     * <p/>
     * This allows us to implement section 1.6 of the functional specification and deal with the cases where an award is being assigned a
     * new sponsored program id from SAP.
     * <p/>
     * TODO for now assume one-to-one mapping between sponsored program message and passed in sponsored programs?
     *
     * @param transmission              the original award transmission data
     * @param sponsoredProgramsMessages the sponsored programs message responses from the SAP service
     * @return a populated Map containing the award id as the key and it's corresponding sponsored program id as the value
     */
    private Map<Long, String> extractSponsoredProgramIds(List<Long> interfacedSponsoredProgramIds, SapTransmission transmission, SPONSOREDPROGRAMSMESSAGES sponsoredProgramsMessages) {
        Map<Long, String> sponsoredProgramIds = new HashMap<Long, String>();
        if (sponsoredProgramsMessages != null && sponsoredProgramsMessages.getItem() != null) {
            if (sponsoredProgramsMessages.getItem().size() != interfacedSponsoredProgramIds.size()) {
                throw new IllegalStateException("The number of sponsored program messages returned should be equal to the number of sponsored programs interfaced.  " +
                        "Instead " + interfacedSponsoredProgramIds.size() + " sponsored programs were interfaced and " +
                        sponsoredProgramsMessages.getItem().size() + " sponsored program messages were returned.");
            }
            for (int index = 0; index < sponsoredProgramsMessages.getItem().size(); index++) {
                ZSPPROGRAMMESSAGES sponsoredProgramMessage = sponsoredProgramsMessages.getItem().get(index);
                Long awardId = interfacedSponsoredProgramIds.get(index);
                sponsoredProgramIds.put(awardId, sponsoredProgramMessage.getSPONSOREDPROG());
            }
        }
     ...
     *
     * @param transmission in progress
    ...
    public ValidationResults validate(SapTransmission transmission) {
        ...
        if (transmission.getAward() == null) {
            ...
            checkPrimaryAwardForChanges(transmission.getAward(), validationResults);
            for (Award childAward : transmission.getChildAwards()) {
                checkChildAwardForChanges(transmission.getAward(), childAward, validationResults);
            }

            CustomAwardDataHelper helper = new CustomAwardDataHelper(transmission.getAward());
            if (helper.getLastTransmissionDate() == null) {
                if (transmission.getChildAwards() == null || transmission.getChildAwards().isEmpty()) {
                    ErrorMessageKeys.NO_CHILDREN_AWARD.populate(validationResults);
                }
            }

            // BU Customization ID: N/A mkousheh 20120713 - Add validation to prevent transmission if Parent transaction
            // type is "No Cost Extension" or "Administrative Change" and the selected child's budget status is "to be posted"
            if (transmission.getAward().getAwardTransactionTypeCode().equals(AWARD_TRANSACTION_TYPE_NO_COST_EXTENSION)
                    || transmission.getAward().getAwardTransactionTypeCode().equals(AWARD_TRANSACTION_TYPE_ADMINISTRATION_CHANGE)) {
                for (Award childAward : transmission.getChildAwards()) {

                    AwardBudgetExt abvoe =  getLastBudgetVersion(childAward.getAwardDocument());
                    boolean awardBudgetVersionToBePosted = false;
                    if (abvoe != null) {
                        String budgetStatus = abvoe.getAwardBudgetStatusCode();
                        awardBudgetVersionToBePosted = Constants.BUDGET_STATUS_CODE_TO_BE_POSTED.equalsIgnoreCase(budgetStatus);
                    ...
                    if (awardBudgetVersionToBePosted) {
                        ErrorMessageKeys.PARENT_TRANSACTION_TYPE_NOCOST_EXT_OR_ADMIN_CHANGE.populate(transmission.getAward()
                                .getAwardId(), validationResults, transmission.getAward().getAwardTransactionType()
                                .getDescription(), childAward.getAwardNumber(), transmission.getAward().getSequenceNumber()
                                .toString());
                        break;
                    }
                }
            ...
            if (transmission.getAward().getAccountTypeCode() == 0) {
                ErrorMessageKeys.NO_PARENT_ACCOUNT_TYPE.populate(validationResults);
            }
            if (transmission.getAward().getAwardAmountInfos().size() > 0) {
                String timeAndMoneyDocumentNumber = transmission.getAward().getLastAwardAmountInfo().getTimeAndMoneyDocumentNumber();

                try {
                    if (StringUtils.isNotBlank(timeAndMoneyDocumentNumber)) {
                        TimeAndMoneyDocument timeAndMoneyDocument = (TimeAndMoneyDocument) getDocumentService().getByDocumentHeaderId(timeAndMoneyDocumentNumber);
                        if (timeAndMoneyDocument != null &&
                                timeAndMoneyDocument.getDocumentHeader().hasWorkflowDocument() &&
                                !timeAndMoneyDocument.getDocumentHeader().getWorkflowDocument().isFinal()) {
                            ErrorMessageKeys.TIME_MONEY_NOT_FINAL.populate(validationResults);
                        }
                    }
        ...
        // only proceed with full validation if no validation errors were raised in previous step
     ...
     *
     * @param rootAward        - parent award object
     * @param getLastSuccesful retuns whether there was a successful transmission or not
     * @return last transmission object
     */
    private AwardTransmission getLatestAwardTransmission(Award rootAward, boolean getLastSuccesful) {
        AwardTransmission latestTransmission = null;

        if (getLastSuccesful) {
            for (AwardTransmission awardTransmission : ((AwardExtension) rootAward.getExtension()).getAwardTransmissions()) {
                if (awardTransmission.isSuccess()) {
                    latestTransmission = awardTransmission;
                }
            }
        } else {
            if (((AwardExtension) rootAward.getExtension()).getAwardTransmissions() != null && ((AwardExtension) rootAward.getExtension()).getAwardTransmissions().size() > 0)
                latestTransmission = ((AwardExtension) rootAward.getExtension()).getAwardTransmissions().get(((AwardExtension) rootAward.getExtension()).getAwardTransmissions().size() - 1);
        }
    ...
    // BU Customization ID: N/A mkousheh 20120505 - Return the latest award child that was successfully transmitted
    private AwardTransmissionChild getLatestChildAwardTransmitted(Award rootAward, String awardNumber, boolean getLastSuccesful) {
        AwardTransmission latestTransmissionWithMatchChild = null;

        List<AwardTransmission> awardTransmissionReversed = ((AwardExtension) rootAward.getExtension()).getAwardTransmissions();

        // reverse award transmission list
        Collections.sort(awardTransmissionReversed, new Comparator<AwardTransmission>() {
            public int compare(AwardTransmission awardTransmission1, AwardTransmission awardTransmission2) {
                return ((Long) awardTransmission1.getTransmissionId()).compareTo((Long) awardTransmission2.getTransmissionId());
            }
        });
        ...
        if (getLastSuccesful) {
            for (AwardTransmission awardTransmission : (List<AwardTransmission>) awardTransmissionReversed) {
                //for (AwardTransmission awardTransmission : rootAward.getAwardTransmissions()) {

                if (awardTransmission.isSuccess()) {
                    for (AwardTransmissionChild awardTransmissionChild : awardTransmission.getTransmissionChildren()) {
            ...
            if (((AwardExtension) rootAward.getExtension()).getAwardTransmissions() != null && ((AwardExtension) rootAward.getExtension()).getAwardTransmissions().size() > 0) {
                // latestTransmission = rootAward.getAwardTransmissions().get(rootAward.getAwardTransmissions().size() - 1);
                for (AwardTransmissionChild awardTransmissionChild : ((AwardExtension) rootAward.getExtension()).getAwardTransmissions()
                        .get(((AwardExtension) rootAward.getExtension()).getAwardTransmissions().size() - 1).getTransmissionChildren()) {
                    if (awardNumber.equals(awardTransmissionChild.getAwardNumber())) {
                        return awardTransmissionChild;
                    }
                }
    ...
    private void checkPrimaryAwardForChanges(Award award, ValidationResults validationResults) {
        AwardTransmission lastTransmission = getLatestAwardTransmission(award, true);

        if (award != null && lastTransmission != null && lastTransmission.isSuccess()) {
            if (award.getAccountTypeCode() != null && lastTransmission.getAccountTypeCode() != null &&
                    award.getAccountTypeCode().compareTo(lastTransmission.getAccountTypeCode()) != 0) {
                ErrorMessageKeys.PARENT_ACCOUNT_TYPE_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmission.getAccountTypeCode().toString(), award.getAccountTypeCode().toString());
            }

            // BU Customization ID: N/A mkousheh 20121130 - Issue 82 only prevent changing Payment
            // Basis Cost reimbursement (Resource Related Billing) (1) to Milestone (4) or vice-versa
            if ((RRB_BASIS_OF_PAYMENT.equals(award.getBasisOfPaymentCode()) && MILESTONE_BASIS_OF_PAYMENT.equals(lastTransmission.getBasisOfPaymentCode()))
                    || (MILESTONE_BASIS_OF_PAYMENT.equals(award.getBasisOfPaymentCode()) && RRB_BASIS_OF_PAYMENT.equals(lastTransmission.getBasisOfPaymentCode()))) {
                ErrorMessageKeys.PARENT_PAYMENT_BASIS_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmission.getBasisOfPaymentCode(), award.getBasisOfPaymentCode());
            }

                    
/*
            if (!MANUAL_BASIS_OF_PAYMENT.equalsIgnoreCase(lastTransmission.getBasisOfPaymentCode())
                    && !StringUtils.equals(award.getBasisOfPaymentCode(), lastTransmission.getBasisOfPaymentCode())) {
                ErrorMessageKeys.PARENT_PAYMENT_BASIS_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmission.getBasisOfPaymentCode(), award.getBasisOfPaymentCode());
            }
*/

            if (DHHS_LOC_MATHOD_OF_PAYMENT.equalsIgnoreCase(lastTransmission.getMethodOfPaymentCode()) && !StringUtils.equals(award.getMethodOfPaymentCode(), lastTransmission.getMethodOfPaymentCode())) {
                ErrorMessageKeys.PARENT_PAYMENT_METHOD_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmission.getMethodOfPaymentCode(), award.getMethodOfPaymentCode());
            }
            if (!StringUtils.equals(award.getSponsorCode(), lastTransmission.getSponsorCode())) {
                ErrorMessageKeys.PARENT_SPONSOR_CODE_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmission.getSponsorCode(), award.getSponsorCode());
            }
        }
    }

    private AwardTransmissionChild findMatchingTransmissionChild(List<AwardTransmissionChild> transmissionChildren, String awardNumber) {
    ...
    private void checkChildAwardForChanges(Award primaryAward, Award award, ValidationResults validationResults) {
        // BU Customization ID: N/A mkousheh 20120504 - Return the latest award child that was successfully transmitted
        AwardTransmissionChild lastTransmittedChild = getLatestChildAwardTransmitted(primaryAward, award.getAwardNumber(), true);
        if (award != null && lastTransmittedChild != null) {
            if (!"group".equalsIgnoreCase(((AwardExtension) award.getExtension()).getChildType()) && !StringUtils.equals(((AwardExtension) award.getExtension()).getChildType(), lastTransmittedChild.getChildType())) {
                ErrorMessageKeys.CHILD_CHILD_TYPE_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmittedChild.getChildType(), ((AwardExtension) award.getExtension()).getChildType());
            }
            if (!StringUtils.equals(award.getLeadUnitNumber(), lastTransmittedChild.getLeadUnitNumber())) {
                ErrorMessageKeys.CHILD_LEAD_UNIT_CANNOT_CHANGE.populate(award.getAwardId(), validationResults, award.getAwardNumber(), lastTransmittedChild.getLeadUnitNumber(), award.getLeadUnitNumber());
            }
        }
    }

    private void validateAward(ValidationResults results, Award award, boolean parentAward) {
        // BU Customization ID: N/A mkousheh N/A - RSN:8311755 (remove validation)
        ...
        // BUKC-0127: Remove validation for cost-share on child award (ENHC0012984)
    ...
    private void validateAwardIsFinal(ValidationResults results, Award award) {
        ...
        VersionHistoryService vhs = (VersionHistoryService) KcServiceLocator.getService(VersionHistoryService.class);
        try {
            VersionHistory versionHistory = vhs.findPendingVersion(Award.class, award.getAwardNumber(), new Integer(award.getSequenceNumber() + 1).toString());
            if (versionHistory != null) {
                Award activeAward = (Award) versionHistory.getSequenceOwner();
                if (activeAward != null) {
                    awardDocument = (AwardDocument) getDocumentService().getByDocumentHeaderId(activeAward.getAwardDocument().getDocumentNumber());
                    if (awardDocument.getDocumentHeader().hasWorkflowDocument()) {
                        if (!awardDocument.getDocumentHeader().getWorkflowDocument().isFinal()) {
                            ErrorMessageKeys.AWARD_NOT_FINAL.populate(award.getAwardId(), results, activeAward.getAwardNumber());
                        }
                    } else {
                        ErrorMessageKeys.AWARD_DOCUMENT_NULL.populate(award.getAwardId(), results, activeAward.getAwardNumber());
                    }
                } else {
                    // show error that there is no active award?
                    ErrorMessageKeys.AWARD_NOT_FINAL.populate(award.getAwardId(), results, activeAward.getAwardNumber());
                }
            } else if (award.getAwardDocument().getDocumentHeader().hasWorkflowDocument() && !award.getAwardDocument().getDocumentHeader().getWorkflowDocument().isFinal()) {
                ErrorMessageKeys.AWARD_NOT_FINAL.populate(award.getAwardId(), results, award.getAwardNumber());
            }
        } catch (WorkflowException e) {
            e.printStackTrace();
        }
    ...
    private void validateAwardDFAFSNumber(ValidationResults results, Award award) {
        /*
        Integer methodOfPaymentCode = 0;
        try {
            methodOfPaymentCode = Integer.parseInt(award.getMethodOfPaymentCode());
        } finally {
            // do nothing
        }
        */
        ...
        if (isLocMethodOfPayment(award.getMethodOfPaymentCode()) && StringUtils.isBlank(dfafsNumber)) {
            // if (methodOfPaymentCode.compareTo(27) <= 0 && StringUtils.isBlank(dfafsNumber)) {
            ErrorMessageKeys.AWARD_DOCUMENT_NO_DOCUMENT_NUMBER.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }

    private boolean isLocMethodOfPayment(String methodOfPaymentCode ) {
        String locMethodOfPaymentList = parameterService.getParameterValueAsString(AwardDocument.class,
     ...
     */
    private void validateChildHasBudget(ValidationResults results, Award award, boolean parentAward) {
        if (!parentAward) {
        	AwardBudgetExt budget = award.getAwardDocument().getBudgetVersionOverview();
            if (budget.getBudgetId() == null) {
                ErrorMessageKeys.NO_BUDGET_ON_CHILD_AWARD.populate(award.getAwardId(), results, award.getAwardNumber());
            }
        }
    }

    private void validateChildAwardType(ValidationResults results, Award award, boolean isParentAward) {
        if (!isParentAward && StringUtils.isBlank(((AwardExtension) award.getExtension()).getChildType())) {
            ErrorMessageKeys.NO_CHILD_TYPE_ON_CHILD_AWARD.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }
     ...
     * <li>Account Type is Federal, and a non-Federal Sponsor is attached; Federal Account Types should only have Federal Sponsors.</li>
     * <li>Account Type is Non-Federal, and a Federal Sponsor is attached; Federal Account Types should only have Federal Sponsors.</li>
     * </ol>
     */
    private void validateAccountType(ValidationResults results, Award award) {
        if (award.getAccountTypeCode() == null) {
            ErrorMessageKeys.ACCOUNT_TYPE_NULL.populate(award.getAwardId(), results, award.getAwardNumber());
        } else {
            if (isFederalAccount(award) && isNonFederalSponsor(award)) {
                ErrorMessageKeys.FEDERAL_ACCOUNT_NON_FEDERAL_SPONSOR.populate(award.getAwardId(), results, award.getAwardNumber());
            } else if (isNonFederalAccount(award) && isFederalSponsor(award)) {
                ErrorMessageKeys.NON_FEDERAL_ACCOUNT_FEDERAL_SPONSOR.populate(award.getAwardId(), results, award.getAwardNumber());
            }
        }
    }
    ...
    // BUKC-0035: Fix an issue with the code to compare the same data type for Federal/NonFederal issue
    private boolean isFederalSponsor(Award award) {
        // BUKC-0064: If Prime Sponsor present, validate against it rather that the Sponsor
        if (!(award.getPrimeSponsor() == null)){
    ...
    private boolean isNonFederalSponsor(Award award) {
        // BUKC-0029: Fix an issue with Acount Type validation (Federal/Non-Federal validation)
        // return true if Sposor is (non) federal - all should be consider non federal if type <> 1
        // return award.getSponsor().getSponsorTypeCode().equals(NON_FEDERAL_CODE);
        return (!isFederalSponsor(award));
    }
    /**
     * Validates if Dollar Amounts Appear on an Award Record that has Children; Dollar amounts can only appear
     * on child awards (that have no children of their own).
     */
    private void validateDollarAmmount(ValidationResults results, Award award) {
        AwardHierarchy awardHierarchy = award.getAwardHierarchyService().loadAwardHierarchy(award.getAwardNumber());
        if (awardHierarchy == null) {
            ErrorMessageKeys.AWARD_HIERARCHY_NULL.populate(award.getAwardId(), results, award.getAwardNumber());

        }

        // BU Customization ID: N/A mkousheh N/A - Based on Eddie's request
/*
        } else if (awardHierarchy.hasChildren() && award.calculateObligatedDistributedAmountTotal().isGreaterThan(ScaleTwoDecimal.ZERO)) {
            ErrorMessageKeys.AWARD_DOLLAR_AMOUNT_ON_CHILD.populate(award.getAwardId(), results, award.getAwardNumber());
        }
*/
    }

    /**
     * Determines if the Lead Unit on the Award is invalid; The Lead Unit can only be populated with a
     * Child Node in the Unit Hierarchy.
     */
    private void validateLeadUnit(ValidationResults results, Award award) {
        List<Unit> subUnits = unitService.getSubUnits(award.getLeadUnitNumber());
        if (subUnits != null && !subUnits.isEmpty()) {
            ErrorMessageKeys.INVALID_LEAD_UNIT.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }
     ...
     * <p/>
     * <ol>
     * <li>Sub-Award does not have a Prime Sponsor Listed; When the Award Type is listed as a Sub-award it needs to have a Prime Sponsor attached</li>
     * -    *   <li>Multiple Subawards are recorded on a Single Award; Only one subaward should be listed on an award.</li>
     * <li>Subawards are recorded on a Parent Award; Subawards can only be listed on child awards.</li>
     * </ol>
     */
    private void validateSubAwards(ValidationResults results, Award award, boolean parentAward) {

        if (parentAward && isSubAward(award) && StringUtils.isBlank(award.getPrimeSponsorCode())) {
            ErrorMessageKeys.SUB_AWARD_NO_PRIME_SPONSOR.populate(award.getAwardId(), results, award.getAwardNumber());
        }

        // BU Customization ID: N/A mkousheh N/A - Per RSN:8311717
        // List<AwardApprovedSubaward> subAwards = award.getAwardApprovedSubawards();
        // if (subAwards != null && subAwards.size() > 1) {
        //    ErrorMessageKeys.MULTIPLE_SUBAWARDS.populate(award.getAwardId(), results, award.getAwardNumber());
        // }
        // if (parentAward && subAwards != null && !subAwards.isEmpty()) {
        //   ErrorMessageKeys.SUBAWARD_ON_PARENT.populate(award.getAwardId(), results, award.getAwardNumber());
        // }
     ...
     *
     * @param award the Award to check to determine if it is a sub-award or not
     * @return true if the given award is a sub-award, false if not
     */
    private boolean isSubAward(Award award) {
        return AWARD_TYPE_CODE_SUBAWARD.equals(award.getAwardType().getCode());
    }
     ...
     * <p/>
     * <ol>
     * <li>Obligation End Date is greater than Project End Date; Obligation End Date should always be equal to or less than the Project End Date.</li>
     * </ol>
     */
    private void validateProjectDates(ValidationResults results, Award award) {
        ...
        java.util.Date projectStartDate = zeroOutTime(award.getAwardEffectiveDate());
        java.util.Date projectEndDate = zeroOutTime(award.getProjectEndDate());
        java.util.Date obligationStartDate = zeroOutTime(awardHierarchyNode.getCurrentFundEffectiveDate());
        java.util.Date obligationEndDate = zeroOutTime(award.getObligationExpirationDate());

        if (projectStartDate == null) {
            ErrorMessageKeys.PROJECT_START_DATE_NULL.populate(award.getAwardId(), results, award.getAwardNumber());
        }

        if (obligationStartDate == null) {
            ErrorMessageKeys.OBLIGATION_START_DATE_NULL.populate(award.getAwardId(), results, award.getAwardNumber());
        }

        if (projectEndDate == null) {
            ErrorMessageKeys.PROJECT_START_END_NULL.populate(award.getAwardId(), results, award.getAwardNumber());
        }

        if (obligationEndDate == null) {
            ErrorMessageKeys.OBLIGATION_END_DATE_NULL.populate(award.getAwardId(), results, award.getAwardNumber());
        }

    }

    /**
     * Performs the (somewhat convoluted) logic to extract an AwardHierarchyNode from an Award.  This code was largely reverse engineered
     * by looking at {@link AwardHierarchyUIServiceImpl}.
     *
     * @param award the award to extract the AwardHierarchyNode from
    ...
    private AwardHierarchyNode getAwardHierarchyNode(Award award) {
        ...
        Map<String, AwardHierarchy> awardHierarchyItems = getAwardHierarchyService().getAwardHierarchy(award.getAwardNumber(), new ArrayList<String>());
        getAwardHierarchyService().populateAwardHierarchyNodes(awardHierarchyItems, awardHierarchyNodes, award.getAwardNumber(), award.getSequenceNumber().toString());
        return awardHierarchyNodes.get(award.getAwardNumber());
    }

    /**
     * Takes the given Date and returns a copy of the Date which has all of the time portions of the date (hours, minutes, seconds, etc.) set to zero.
     *
     * @param givenDate the date to zero out
    ...
    /**
     * Validates if Cost Sharing Appears on a Child Award; Cost Sharing can only be represented at the Parent..
     *
     * @param results
     * @param award
     */
    private void validateCostSharingOnChildAward(ValidationResults results, Award award, boolean parentAward) {
        if (!parentAward && isCostSharing(award)) {
            ErrorMessageKeys.COST_SHARING_ON_CHILD_AWARD.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }

    private void validateGroupAwardHasChildren(ValidationResults results, Award award) {
        AwardHierarchy hierarchy = getAwardHierarchyService().loadAwardHierarchy(award.getAwardNumber());

        if ("Group".equalsIgnoreCase(((AwardExtension) award.getExtension()).getChildType()) && !hierarchy.hasChildren()) {
            ErrorMessageKeys.GROUP_AWARD_HAS_NO_CHILDREN.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }

    /**
     * Validates if Federal Award does not have a CFDA Number; When the Account Type is Federal the CFDA Number is required.
     *
     * @param results
     * @param award
     */
    private void validateFederalAward(ValidationResults results, Award award) {
        if (isFederalAccount(award) && StringUtils.isBlank(award.getCfdaNumber())) {
            ErrorMessageKeys.CFDA_NUMBER_REQUIRED_FEDERAL_ACCOUNT.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }


    /**
     * Validates if NIH Award does not have a Document Number; When an NIH sponsor is listed the Document Number should not be Null.
     *
     * @param results
     * @param award
     */
    private void validateNIHAward(ValidationResults results, Award award) {
        ...
        if (isNih && StringUtils.isBlank(award.getDocumentFundingId())) {
            ErrorMessageKeys.NIH_AWARD_NO_DOCUMENT_NUMBER.populate(award.getAwardId(), results, award.getAwardNumber());
        }

    }

    /**
     * This is a validation that was originally spec'd.
     * This Validation should trigger if Award.Award_Type = 6 and Award.Prime_Sponsor_Code is NULL
     *
     * @param results
     * @param award
     */
    private void validatePrimeSponsorOnSubAward(ValidationResults results, Award award) {
        if (AWARD_TYPE_CODE_SUBAWARD == award.getAwardTypeCode() && StringUtils.isBlank(award.getPrimeSponsorCode())) {
            ErrorMessageKeys.SUB_AWARD_WITH_NO_PRIME_SPONSOR.populate(award.getAwardId(), results, award.getAwardNumber());
        }
    }

    /**
     * This is a validation that was originally spec'd.
     * This validation should trigger if the parent award has an amount > 0 the
    ...
    private void validateDollarObligation(ValidationResults results, Award award) {
        AwardAmountInfo awardAmountInfo = getAwardAmountInfoService().fetchLastAwardAmountInfoForAwardVersionAndFinalizedTandMDocumentNumber(award);
        AwardHierarchy hierarchy = getAwardHierarchyService().loadAwardHierarchy(award.getAwardNumber());
        if (hierarchy.hasChildren() && awardAmountInfo.getObliDistributableAmount().isPositive()) {
//            ErrorMessageKeys.DOLLARS_OBLIGATED_ON_AWARD_WITH_NO_CHILDREN.populate(award.getAwardId(), results, award.getAwardNumber());
    ...
    /**
     * An enumeration which defines all of the error messages that can be produced by validation of an
     * SAP award transmission.
     */
    private static enum ErrorMessageKeys {

        AWARD_DOCUMENT_NULL("error.award.sapintegration.awardDocumentNull"),
        AWARD_NOT_FINAL("error.award.sapintegration.awardMustBeFinal"),
        TIME_MONEY_NOT_FINAL("error.award.sapintegration.timeAndMoneyDocumentMustBeFinal"),
        NO_PARENT_ACCOUNT_TYPE("error.award.sapintegration.noParentAccountType"),
        NO_PARENT_AWARD("error.award.sapintegration.noParent"),
        NO_CHILDREN_AWARD("error.award.sapintegration.noChildren"),
        NO_CHILD_TYPE_ON_CHILD_AWARD("error.award.sapintegration.noChildType"),
        NO_BUDGET_ON_CHILD_AWARD("error.award.sapintegration.noBudgetOnChild"),
        ACCOUNT_TYPE_NULL("error.award.sapintegration.nullAccountType"),
        AWARD_HIERARCHY_NULL("error.award.sapintegration.nullHierarchy"),
        AWARD_DOLLAR_AMOUNT_ON_CHILD("error.award.sapintegration.dollarAmountOnChild"),
        FEDERAL_ACCOUNT_NON_FEDERAL_SPONSOR("error.award.sapintegration.federalAccount.nonFederalSponsor"),
        NON_FEDERAL_ACCOUNT_FEDERAL_SPONSOR("error.award.sapintegration.nonFederalAccount.federalSponsor"),
        MULTIPLE_SUBAWARDS("error.award.sapintegration.subaward.multiple"),
        SUBAWARD_ON_PARENT("error.award.sapintegration.subawardOnParent"),
        SUB_AWARD_NO_PRIME_SPONSOR("error.award.sapintegration.subaward.noPrimeSponsor"),
        INVALID_LEAD_UNIT("error.award.sapintegration.invalid.leadUnit"),
        PROJECT_START_DATE_NULL("error.award.sapintegration.projectStartDate.required"),
        PROJECT_START_END_NULL("error.award.sapintegration.projectEndDate.required"),
        OBLIGATION_START_DATE_NULL("error.award.sapintegration.obligationStartDate.required"),
        OBLIGATION_END_DATE_NULL("error.award.sapintegration.obligationEndDate.required"),
        OBLIGATION_END_DATE_AFTER_PROJECT_END_DATE("error.award.sapintegration.obligationEndDate.after.projectEndDate"),
        COST_SHARING_ON_CHILD_AWARD("error.award.sapintegration.costSharing.onChildAward"),
        CFDA_NUMBER_REQUIRED_FEDERAL_ACCOUNT("error.award.sapintegration.cfdaNumber.requiredOn.federalAccount"),
        NIH_AWARD_NO_DOCUMENT_NUMBER("error.award.sapintegration.nihAward.noDocumentNumber"),
        DOLLARS_OBLIGATED_ON_AWARD_WITH_NO_CHILDREN("error.award.sapintegration.dollarsObligatedOnAwardWithChildren"),
        SUB_AWARD_WITH_NO_PRIME_SPONSOR("error.award.sapintegration.subAwardWithNoPrimeSponsor"),
        PARENT_ACCOUNT_TYPE_CANNOT_CHANGE("error.award.sapintegration.parentAward.accountTypeChanged"),
        PARENT_SPONSOR_CODE_CANNOT_CHANGE("error.award.sapintegration.parentAward.sponsorCodeChanged"),
        PARENT_PAYMENT_METHOD_CANNOT_CHANGE("error.award.sapintegration.parentAward.paymentMethodChanged"),
        PARENT_PAYMENT_BASIS_CANNOT_CHANGE("error.award.sapintegration.parentAward.paymentBasisChanged"),
        CHILD_CHILD_TYPE_CANNOT_CHANGE("error.award.sapintegration.childAward.childTypeChanged"),
        CHILD_LEAD_UNIT_CANNOT_CHANGE("error.award.sapintegration.childAward.leadUnitChanged"),
        GROUP_AWARD_HAS_NO_CHILDREN("error.award.sapintegration.groupAward.hasNoChildren"),
        AWARD_DOCUMENT_NO_DOCUMENT_NUMBER("error.award.sapintegration.awardDocument.noDocumentNumber"),
        PARENT_TRANSACTION_TYPE_NOCOST_EXT_OR_ADMIN_CHANGE("error.award.sapintegration.parentAward.transactionType.noCostExt_Or_administrativeChange")
        ;
        ...
        //       private void populate(Long awardId, ValidationResults validationResults, String parentAwardTransactionType, String childAwardNumber) {
        //           ValidationError validationError = new ValidationError(errorMessageKey, Arrays.asList(parentAwardTransactionType, childAwardNumber));
        //           validationResults.addAwardValidationError(awardId, validationError);
//        }

//        private void populate(Long awardId, ValidationResults validationResults, String awardNumber, String oldValue, String newValue) {
//            ValidationError validationError = new ValidationError(errorMessageKey, Arrays.asList(awardNumber, oldValue, newValue));
//            validationResults.addAwardValidationError(awardId, validationError);
//        }


        private void populate(Long awardId, ValidationResults validationResults, String param1, String param2, String param3) {
            ValidationError validationError = new ValidationError(errorMessageKey, Arrays.asList(param1, param2, param3));
            validationResults.addAwardValidationError(awardId, validationError);
        }


        private void populate(Long awardId, ValidationResults validationResults, String awardNumber) {
            ValidationError validationError = new ValidationError(errorMessageKey, new ArrayList<String>(Arrays.asList(awardNumber)));
            validationResults.addAwardValidationError(awardId, validationError);
        }

        private void populate(ValidationResults validationResults) {
            ValidationError validationError = new ValidationError(errorMessageKey, new ArrayList<String>());
    ...
    /**
     * Maps the given KC sponsor's account type code to SAP grant type code as per "Mapping Table M1" in the specification.
     */
    private String convertAccountTypeToGrantType(int accountType, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.ACCOUNT_TYPE_MAPPING, Integer.toString(accountType));
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert account type, given account type value was not understood: " + accountType + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private static enum FringeCodeMapping {
        ...
        private static String mapToSapFringeCode(boolean isFringeNotAllowed, Integer accountTypeCode) {
    ...
    /**
     * Defines the mapping from KC activity types to SAP functional areas as per "Mapping Table M7" in the specification.
     */
    private String convertActivityTypeToFunctionalArea(String activityType, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.ACTIVITY_TYPE_MAPPING, activityType);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert activity type, given activity type value was not understood: " + activityType + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }

    /**
     * Defines the mapping from KC Basis of Payment code to SAP Billing Rule as per "Mapping Table M3" in the specification.
     */
    private String convertBasisOfPaymentToBillingRule(String basisOfPaymentCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.BASIS_OF_PAYMENT_MAPPING, basisOfPaymentCode);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert basis of payment code, given basis of payment value was not understood: " + basisOfPaymentCode + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }

    /**
     * Defines the mapping from KC Method of Payment code to SAP Letter of Credit as per "Mapping Table M4" in the specification.
     */
    private String convertMethodOfPaymentToLetterOfCredit(String methodOfPaymentCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.METHOD_OF_PAYMENT_MAPPING, methodOfPaymentCode);
    ...
    /**
     * Defines the mapping from KC project role to SAP responsibility as per "Mapping Table M6" in the specification.
     */
    private String convertProjectRoleToResponsibility(String projectRole, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.PROJECT_ROLE_MAPPING, projectRole);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert project role, given project role value was not understood: " + projectRole + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }

    /**
     * Defines the mapping from KC status code to SAP status code as per "Mapping Table M5" in the specification.
     */
    private String convertStatusCodeToResponsibility(int statusCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.STATUS_CODE_MAPPING, Integer.toString(statusCode));
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert status code, given status code value was not understood: " + statusCode + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
     ...
     */
    private String convertSponsorCodeToSponsorType(String sponsorCode, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.SPONSOR_CODE_MAPPING, sponsorCode);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert sponsor code, given sponsor code value was not understood: " + sponsorCode + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }

    /**
     * Defines the mapping from Sponsor Terms in KCRM to Sponsor Class in SAP as per functional specification mapping table M8.
     */
    private String convertSponsorTermToSponsorClass(long sponsorTerm, Award award) {
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.SPONSOR_TERM_MAPPING, Long.toString(sponsorTerm));
        if (StringUtils.isEmpty(returnValue)) {
            LOG.info("[awardId: " + award.getAwardId() + ", awardNumber: " + award.getAwardNumber() + "] - " + "Failed to convert sponsor term, given sponsor term value was not understood: " + sponsorTerm);
        }
        return returnValue;
    }
    ...
    private String convertLeadUnitToBusinessArea(String leadUnitNumber, Award award) {
        ...
        String returnValue = getParameterService().getSubParameterValueAsString(AwardDocument.class, BUConstants.LEAD_UNIT_MAPPING, firstDigit);
        if (StringUtils.isEmpty(returnValue)) {
            throw new IllegalArgumentException("Failed to convert lead unit, given lead unit value was not understood: " + leadUnitNumber + " awardNumber was: " + award.getAwardNumber() + "\n" + award.toString());
        }
        return returnValue;
    }
    ...
    private void logAwardInfo(Award award, String message) {
        if (LOG.isInfoEnabled()) {
            LOG.info("[awardId: " + award.getAwardId() + ", awardNumber: " + award.getAwardNumber() + "] - " + message);
        }
    }
    ...
    public void setBudgetRateAndBaseService(BudgetRateAndBaseService budgetRateAndBaseService) {
     ...
     *
     * @param award on progress to be transmitted
    ...
    protected AwardBudgetExt getLastBudgetVersion(AwardDocument awardDocument) {
        ...
        if (versionSize > 0) {
            budgetVersionOverview = awardBudgetDocumentVersions.get(versionSize - 1).getFirstBudget();
        }
        return budgetVersionOverview;
    }
}

class AwardTransmissionComparator implements Comparator<AwardTransmission>, Serializable {
