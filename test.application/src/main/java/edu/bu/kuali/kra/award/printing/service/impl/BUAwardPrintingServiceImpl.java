...
import edu.bu.kuali.kra.award.printing.*;
...
public class BUAwardPrintingServiceImpl implements BUAwardPrintingService {
     ...
     *
     * @param reportParameters objects
     * @return awardPrintDO
     */
     ...
     *
     * @param award            in progress
     * @param timeAndMoneyList selected T&M list
     * @return awardPrintDO
     */
     ...
     *
     * @param award to generate the notification for
    ...
    private List<BUAwardPrintTerm> populateAwardPrintTerms(Award award) {
        ...
        String suppressedTermIds = parameterService.getParameterValueAsString(AwardDocument.class,
                BUConstants.AWARD_NOTIFICATION_SUPRESS_TERM_ID);
        String suppressedTermCds = parameterService.getParameterValueAsString(AwardDocument.class,
                BUConstants.AWARD_NOTIFICATION_SUPRESS_TERM_CD);
     ...
     * comma delimiter
     *
     * @param suppressedTermIds       Terms IDs as a string with comma delimiter
     * @param suppressedTermCodes     Term Codes as a string with comma delimiter
     * @param suppressedTermTypeCodes Term Type Codes as a string with comma delimiter
    ...
    private Map<Long, String[]> buildSuppressed(String suppressedTermIds, String suppressedTermCodes, String suppressedTermTypeCodes) {
        ...
        for (int i = 0; i < termIds.length; i++) {
            suppressed.put(Long.parseLong(termIds[i]), new String[]{termCodes[i], termTypeCodes[i]});
        }
     ...
     *
     * @param suppressed list
     * @param termId     current term Id
     * @param termCd     current term code
     * @param termTypeCd current term type code
     ...
     *
     * @param timeAndMoneyList the selected T&M list
    ...
    private List<BUAwardPrintAdmin> populateBUAwardPrintOSPAdmin(Award award) {
        ...
        List<UnitAdministrator> unitAdministrators = (List<UnitAdministrator>) getBusinessObjectService().findMatching(
                UnitAdministrator.class, fieldValues);
    ...
    private List<BUAwardPrintAdmin> getListOfPafoOrOspAdminsOnUnitContact(Award award, AdminStatus type) {
        ...
        List<AwardUnitContact> matchingContacts = (List<AwardUnitContact>) getBusinessObjectService().findMatching(
                AwardUnitContact.class, fieldValues);
    ...
    private BUAwardPrintAntAmt populateBUAwardPrintAntAmt(Award award) {
        ...
        AwardAmountInfo newestAAI = awardAmountInfoService
        ...
        List<AwardDirectFandADistribution> fnaDistributionList = (List<AwardDirectFandADistribution>) businessObjectService
                .findMatching(AwardDirectFandADistribution.class, fieldValues);
        ...
        if (timeAndMoneyList != null) {
            for (TimeAndMoneyDocument timeAndMoneyDocument : timeAndMoneyList) {
                ...
                List<AwardAmountInfo> awardAmountInfoList =
                        (List<AwardAmountInfo>) businessObjectService.findMatching(AwardAmountInfo.class, fieldValues);
     ...
     *
     * @param awardAmountInfoList aai list
     */
...
//            if (awardAmountInfo.getSequenceNumber() == seqNum && (!awardAmountInfo.getAwardNumber().endsWith("-00001"))) {
     ...
     *
     * @param aai          aai object
     * @param printObjects print objects
     */
    ...
    private ScaleTwoDecimal getMaxFAndARate(Award award) {
        ...
        if (!(latestBudgetVersionOverview == null)) {
            ...
            List<BudgetRateAndBase> budgetRateAndBaseList = (List<BudgetRateAndBase>) businessObjectService.findMatching(
                    BudgetRateAndBase.class, fieldValues);
     ...
     *
     * @param parameterService The businessObjectService to set.
     */
     ...
     * Sets the businessObjectService attribute value.
     *
     * @param awardAmountInfoService The businessObjectService to set.
     */
