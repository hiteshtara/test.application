...
public class SubAwardHomeAction extends SubAwardAction{
   ...
   getWorkflowDocument());
    ...
    boolean addFundingSourceToSubAward(
        ...
        // BUKC-0168: Populate Subaward Title from Award's Title when linked (DFCT0011711)
        subAward.setTitle(fundingSources.getAward().getTitle());
    ...
    throws Exception {
        ...
        // BUKC-0168: Populate Subaward Title from Award's Title when linked (DFCT0011711)
        if (subAwardDocument.getSubAward().getSubAwardFundingSourceList().isEmpty()) {
            subAwardDocument.getSubAward().setTitle(null);
        } else {
            subAwardDocument.getSubAward().setTitle(subAwardDocument.getSubAward().getSubAwardFundingSourceList().get(subAwardDocument.getSubAward().getSubAwardFundingSourceList().size() - 1).getAward().getTitle());
        }
        return mapping.findForward(Constants.MAPPING_SUBAWARD_PAGE);
    }
